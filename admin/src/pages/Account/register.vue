...
const registerInfo = ref<RegisterViewModel>({
  username: '',
  email: '',
  password: '',
})
  ...
  email: [
    {
      ...
      type: 'email',
      trigger: ['blur', 'change'],
      message: t('register.emailValidation'),
    },
  ],
  password: [
    {
      ...
      <div class="md:shadow-lg bg-white dark:bg-slate-800 rounded-md w-full">
        <div class="p-5">
          ...
          <n-form ref="formRef" :model="registerInfo" :rules="rules" @submit.prevent="register()">
            ...
            <n-form-item class="mb-1" path="email" :label="t('register.email')">
              <n-input v-model:value="registerInfo.email" type="email" :placeholder="t('register.email')" />
            </n-form-item>
