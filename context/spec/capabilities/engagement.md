...
`content_engagement` feature 消费带 content version 的事实事件，幂等更新投影并支持显式重建。互动排序使用 projection，因此允许短暂最终一致；diagnostics 必须报告 lag、乱序和孤儿投影。

互动 sort allowlist 为 `view_count`、`like_count`、`rating_sum`、`rating_count`、`rating_average`。未传 sort 时沿用 content 置顶默认序。

## 5. HTTP 与权限

- 用户侧：`/api/v1/content/{type_name}`、`/api/v1/content/{type_name}/{content_id}` 及 nested views/like/rating。
- 用户收藏：`GET /api/v1/me/favorites/{type_name}`。
- 管理员只读摘要随 admin content 返回；投影重建是独立、审计的运维 Command。
