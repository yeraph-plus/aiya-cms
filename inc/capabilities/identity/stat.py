"""Identity capability statistics provider."""
...
async def stat(*, uow_factory: Any, now: Any, window_days: int = 7) -> dict[str, Any]:
    return await count_created(
        uow_factory=uow_factory, model=IdentityUser, now=now, window_days=window_days
    )
...
class Provider:
    ...
    async def summary(self, *, window: str = "7d") -> dict[str, Any]:
        ...
        return await stat(
            uow_factory=self._uow_factory, now=self._clock.utc_now(), window_days=days
        )
