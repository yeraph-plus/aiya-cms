...
export type CommentDTO = components['schemas']['AdminCommentDTO'];
export type CommentPageDTO = components['schemas']['Page_AdminCommentDTO_'];
...
export async function approveComment(commentId: string, signal?: AbortSignal): Promise<CommentDTO> {
    return getApi().post(
        apiPath('/api/v1/admin/comments/{comment_id}/approve', {
...
export async function rejectComment(commentId: string, body: RejectCommentInput, signal?: AbortSignal): Promise<CommentDTO> {
    return getApi().post(
        apiPath('/api/v1/admin/comments/{comment_id}/reject', {
            comment_id: commentId
        }),
        body,
        { signal }
...
export async function deleteComment(commentId: string, body: DeleteCommentInput = {}, signal?: AbortSignal): Promise<CommentDTO> {
    return getApi().post(
        apiPath('/api/v1/admin/comments/{comment_id}/delete', {
            comment_id: commentId
        }),
        body,
        { signal }
