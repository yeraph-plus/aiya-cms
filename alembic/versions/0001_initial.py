...
def upgrade() -> None:
    ...
    # Engagement is part of the unpublished initial schema.  Keep its
    # projection and fact tables in the single release revision rather than
    # introducing a compatibility branch for a schema that has not shipped.
    op.create_table(
    ...
    op.create_index(
        "ix_engagement_content_stats_type_name",
        "engagement_content_stats",
        ["type_name"],
        unique=False,
    )
    ...
    op.create_index(
        "ix_engagement_content_views_content_id",
        "engagement_content_views",
        ["content_id"],
        unique=False,
    )
    ...
    op.create_index(
        "ix_engagement_content_likes_content_id",
        "engagement_content_likes",
        ["content_id"],
        unique=False,
    )
    op.create_index(
        "ix_engagement_content_likes_subject_id",
        "engagement_content_likes",
        ["subject_id"],
        unique=False,
    )
    ...
    op.create_index(
        "ix_engagement_content_ratings_content_id",
        "engagement_content_ratings",
        ["content_id"],
        unique=False,
    )
    op.create_index(
        "ix_engagement_content_ratings_subject_id",
        "engagement_content_ratings",
        ["subject_id"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    # ### commands auto generated by Alembic - please adjust! ###
    # The engagement projection was folded into this initial release revision
    # before the schema shipped.  A database created by an interrupted or
    # pre-release install can therefore have the revision marker without these
    # optional projection tables.  Drop them defensively so migration-check can
    # still exercise the full downgrade/upgrade cycle.
    op.execute('DROP TABLE IF EXISTS "engagement_content_ratings" CASCADE')
    op.execute('DROP TABLE IF EXISTS "engagement_content_likes" CASCADE')
    op.execute('DROP TABLE IF EXISTS "engagement_content_views" CASCADE')
    op.execute('DROP TABLE IF EXISTS "engagement_content_stats" CASCADE')
    op.drop_index(
        op.f("ix_membership_renewal_records_subscription_id"),
    ...
    op.drop_index(
        op.f("ix_assets_upload_intents_owner_subject_id"),
        table_name="assets_upload_intents",
    )
