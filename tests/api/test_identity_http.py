...
async def test_ban_blocks_bearer_and_delete_marks_status(
    ...
    assert (await client.get("/api/v1/admin/users", headers=user_headers)).status_code == 403
    ...
    assert (await client.get("/api/v1/admin/users", headers=user_headers)).status_code == 401
