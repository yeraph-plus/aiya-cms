---
version: alpha
name: aiya-cms-user-site-design
description: Aiya CMS user-site visual language and element tokens, adapted from the documented editorial enterprise interface reference.
...
本文件定义用户站的视觉语言、Design Tokens、组件元素和响应式视觉行为。页面骨架与信息优先级见 [`LAYOUT.md`](LAYOUT.md)，Astro/FastAPI 边界、路由、认证、SEO 与内容合同见 [`user-site.md`](user-site.md)。本设计仍为 `alpha`：实现可以补齐专有字体替代方案和品牌素材，但不得绕过可访问性、安全渲染或服务端首屏合同。
