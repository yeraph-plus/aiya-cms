...
class GrantPointsResult(TypedDict):
    """Stable result returned by a points grant.

    Membership records the points ledger entry as its reconciliation anchor;
    it deliberately does not consume a computed balance or any points ORM
    object across the Port boundary.
    """

    entry_id: str


class PointsLedgerPort(Protocol):
