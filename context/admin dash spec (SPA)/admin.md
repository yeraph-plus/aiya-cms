...
- 管理员 SPA 的会话与业务 adapter 只能调用 `/api/v1/admin/**`；共同公共面仅允许 `/api/v1/auth/**` 的注册、验证与密码重置和 issuer 下的 OIDC 协议端点。管理员会话固定读取 `/api/v1/admin/session`，不得为了复用用户侧页面调用 `/api/v1/me`、content、community、points、membership、payment 等普通用户业务端点。
...
- OIDC callback 完成后以 bearer token 调用 `GET /api/v1/admin/session` 建立前端会话；响应 capability 集已经由 backend 与当前 manifest permission registry 求交，前端只据此控制导航和按钮，不能恢复未装配或数据库陈旧的 permission key。
...
| 社区 | `/community/discussions` | discussion 查询、详情抽屉、审核、锁定和归档 |
| 社区 | `/community/tags` | community primary/secondary tag、单层层级、顺序和归档管理 |
...
菜单顺序固定为 Dashboard、Content、Community、Users、System、Settings。路由未完成后端契约时不得注册、不得进入菜单，也不得指向 placeholder。
...
- 评论、积分账户和会员订阅的管理员列表保留原始 opaque ID，同时由 API 提供可选 `author`/`target`/`subject` 投影；前端优先显示投影标签，缺失时回退到稳定 ID。
...
- community discussions 与 posts 审核、lock/unlock、archive；列表明确展示 template/status/tags/reply count/last posted time，不借用文章编辑器或 comments 审核页。
- community Tags 独立管理 primary/secondary、单层 parent、position 与 archive；不得调用 taxonomy term API 或共享 taxonomy 表单 adapter。
...
| 定义 | 当前 HTTP 导出 | 管理端约束 |
| --- | --- | --- |
| points program | `/api/v1/admin/points/programs` 提供目录、创建、更新和 activate/deactivate 语义操作；ledger/adjust 仍只接受已注册 `program_key` | `/users/points` 同时展示计划统计和计划管理；编辑保持 `unit` 不可变，写操作带版本与 reason，并受 capability/audit 约束 |
| membership level | `/api/v1/admin/membership/levels` 提供目录、创建、更新和 activate/archive 语义操作；订阅开通只接受 active 等级 | `/settings/membership` 使用独立配置页；归档等级禁止新订阅/续费，写操作带版本与 reason，并受 capability/audit 约束 |
...
- post/page 正文编辑器保存并提交 Markdown 原文；format/profile 来自后端只读 metadata。首版不提供 HTML source、MDX 或把 WYSIWYG HTML 转存为 Markdown 的兼容模式。
- 编辑预览必须复用与 Astro SSR 相同版本的纯 TypeScript `packages/markdown`、profile、sanitize allowlist 和 golden fixtures；不得请求后端生成 HTML，也不得维护第二套 Markdown 方言。
- 预览输出只能通过共享 renderer 返回的内部 `TrustedHtml` 边界挂载；禁止把原始 `body` 或未清洗插件输出直接交给 `v-html`/`innerHTML`。代码块和 embed 不执行。
- 后端返回的 Markdown 校验错误按字段显示安全 message、稳定 code 和 request ID；不得在错误面板或控制台回显大段正文/parser stack。
- post/page 创建表单不提交 slug；创建成功后显示服务端生成的只读 slug 与复制操作。编辑 title 不改变 slug，管理端不得提供“跟随标题更新 URL”开关或手动 slug 覆盖。
...
- `excerpt` 是摘要与 SEO fallback 的唯一基础字段；表单不得同时显示或提交 `PostData.summary`。
...
- 本轮 Compose 拓扑使用 unprivileged Nginx 发布 SPA，并把 `/api/**`、`/oidc/**`、`/.well-known/openid-configuration` 和 `/healthz` 同源反向代理到唯一 backend；外层 TLS/域名由 1panel/Caddy/Nginx 等入口代理负责。浏览器只看到一个 HTTPS 管理员 origin，`AIYA_ISSUER` 与 `AIYA_PUBLIC_BASE_URL` 必须等于该 origin。
- 生产也可使用 `Dockerfile.production` 的单体应用镜像：Nginx 继续托管 `dist`，并只向同一容器内 loopback 的 FastAPI 代理上述路径；这只是部署打包方式变化，不允许让前端直接绕过 Nginx 暴露 API 端口。
- backend 默认只绑定宿主 loopback；管理员静态服务也默认绑定 loopback，生产入口代理是唯一公网入口。若改为 admin 与 API 分离 origin，部署配置必须另行固定 API/issuer URL、CORS allowlist、OIDC redirect/post-logout URI 和 CSP，并重新完成 cookie 同 site 验收。
- sourcemap 发布、缓存策略、安全 header、SPA fallback 和 immutable hashed assets 必须由生产容器/托管平台明确配置。
- `index.html` 与 SPA fallback 使用 `no-store`；带 hash 的 `/assets/**` 使用一年 immutable cache；容器必须配置 CSP、`X-Content-Type-Options`、frame-ancestors/点击劫持、Referrer-Policy 和 Permissions-Policy。
- 本轮 backend 固定选择 `management_plane`；完整 `cms`、Astro 用户站服务和依赖未完成 product feature 的管理工作台不属于本轮上线拓扑，也不得作为已交付页面展示。
...
- Markdown 编辑/预览单元测试与 Astro 共用恶意语料和 golden fixtures，证明 raw HTML、MDX、危险 URL、非法 asset 与脚本执行路径不能进入预览 DOM。
