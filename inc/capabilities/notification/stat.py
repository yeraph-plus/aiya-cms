"""Notification capability statistics provider (available when mounted)."""

from __future__ import annotations

from typing import Any

from inc.capabilities.notification.models import NotificationIntent
...
async def stat(*, uow_factory: Any, now: Any, window_days: int = 7) -> dict[str, Any]:
    return await count_created(
        uow_factory=uow_factory, model=NotificationIntent, now=now, window_days=window_days
    )
