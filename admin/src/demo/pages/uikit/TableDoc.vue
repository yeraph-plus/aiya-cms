...
import { CustomerService, type Customer } from '@/demo/services/CustomerService';
import { ProductService, type Product, type ProductOrder } from '@/demo/services/ProductService';
import { FilterMatchMode, FilterOperator } from '@primevue/core/api';
import type { DataTableFilterMeta, DataTableFilterMetaData } from 'primevue/datatable';
import { computed, onBeforeMount, reactive, ref } from 'vue';

const customers1 = ref<Customer[]>([]);
const customers2 = ref<Customer[]>([]);
const customers3 = ref<Customer[]>([]);
const filters1 = ref<DataTableFilterMeta>({});
const globalFilter1 = computed<string | null>({
    get: () => (filters1.value['global'] as DataTableFilterMetaData).value as string | null,
    set: (value) => {
        (filters1.value['global'] as DataTableFilterMetaData).value = value;
    }
});
...
function getOrderSeverity(order: ProductOrder) {
    switch (order.status) {
        case 'DELIVERED':
            return 'success';
...
function getStockSeverity(product: Product) {
...
function expandAll() {
    expandedRows.value = products.value.reduce<Record<string, boolean>>((acc, p) => {
        acc[p.id] = true;
        return acc;
    }, {});
}
            ...
            <template #header>
                <div class="flex justify-between">
                    ...
                    <IconField>
                        ...
                        <InputText v-model="globalFilter1" placeholder="Keyword Search" />
                    </IconField>
                </div>
            </template>
