...
- release 空库基线生成根 `alembic/versions/0001_initial.py`（revision `release_0001`），包含全部随发行版交付的 kernel/capability 表。
- `release_0001` 发布后不可修改；后续 revision 由对应 owner 放入自身 `migrations/`，并登记到根 manifest。
...
- 实现期间可使用临时 revision；首个 release 基线验收前 squash 为一个 `release_0001`。
- 版本目录只允许保留该一个 deployable revision，comments/community 表、`pg_trgm` 与 trigram/排序索引必须由它一次创建。
